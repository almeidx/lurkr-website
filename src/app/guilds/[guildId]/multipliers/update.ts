	...
	(value) => {
		...
		switch (type) {
			case XpMultiplierType.Global:
				return { id: parts.slice(2).join("-"), multiplier: null, type, key: value };

			case XpMultiplierType.Channel:
			case XpMultiplierType.Role:
				return { id: parts.slice(3).join("-"), multiplier: parse(multiplierValueSchema, parts[2]), type, key: value };
...
export async function update(guildId: string, premium: boolean, data: FormData) {
	...
	await action(guildId, settings, "settings-multipliers");
}
