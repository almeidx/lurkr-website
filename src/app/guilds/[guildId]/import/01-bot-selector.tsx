...
import atlasIcon from "@/assets/bots/atlas.svg";
	...
	return (
		<div className="flex h-6 items-center gap-2 rounded-lg">
			...
			<SelectPopover
				...
				gutter={8}
				sameWidth
				className="z-[10000] flex w-56 flex-col gap-2 rounded-lg bg-light-gray px-3 py-2 shadow-dim-inner"
			>
				{bots.map(({ name, icon }) => (
					<SelectItem
						key={name}
						...
						value={name}
					>
						<Image src={icon} alt={`${name} icon`} width={20} height={20} className="h-5 w-5 rounded-full" />
						{name}
					</SelectItem>
				))}
			</SelectPopover>
