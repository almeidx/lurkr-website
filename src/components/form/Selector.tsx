...
export default function Selector({ id, limit, items, initialItems, onSelect, type, disabled }: SelectorProps) {
	...
	return (
		<div className="relative" ref={elementRef}>
			<div className="flex min-h-[3rem] min-w-[6rem] flex-row flex-wrap gap-1.5 rounded-md bg-discord-not-quite-black px-5 py-3 shadow focus:outline-none">
				{selected.map((item) => (
					<RoleChannelBullet
			...
			>
				...
				<div className="flex max-h-48 w-full flex-col gap-y-0.5 overflow-y-auto">
					{options.map((item) => (
						<div
							className="flex w-full cursor-pointer items-center px-6 py-3 text-left text-white hover:bg-discord-lighter"
							data-id={item.id}
							key={item.id}
