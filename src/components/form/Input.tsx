		...
		<div className={`flex w-full ${props.className ?? ""}`}>
			<div className="relative w-full grow">
				...
				{value && !props.noClearButton && (
					<label
						className="absolute right-0 mx-4 my-auto h-full cursor-pointer py-3 text-2xl text-discord-red transition-colors active:text-red-600"
						onClick={() => {
							setValue("");
							props.onChange("");
						}}
			...
			{"onSubmit" in props && (
				<button
					className="ml-3 flex h-12 w-12 shrink-0 items-center justify-center rounded-md bg-discord-not-quite-black text-white transition-colors hover:text-opacity-75 disabled:select-none"
					disabled={props.disabled}
					onClick={(event) => {
