...
export default function Navbar() {
  ...
  return (
    <header className="flex md:items-center bg-discord-dark p-6">
      <Link href="/">
        <a className=" md:text-xl py-1 md:p-0 text-white uppercase whitespace-nowrap mr-4">Pepe Manager</a>
      </Link>

      <nav className="navbar-links md:w-full ml-auto z-20">
        <button className="w-8 h-8 md:hidden text-white p-1">
        ...
        <nav className="navbar-links-dd hidden md:block absolute md:relative w-full mt-6 md:mt-0 left-0 bg-discord-not-quite-black md:bg-transparent">
          <ul className="flex flex-col md:flex-row py-4 pr-4 md:p-0 gap-4 md:items-center">
            ...
            {authenticated ? (
              ...
              <button
                className="text-white px-2 py-1 mx-2 md:mx-0 bg-blurple rounded-md shadow-md flex flex-row justify-center items-center gap-2 duration-150 hover:bg-lighter-blurple focus:outline-none mt-6 md:mt-0"
