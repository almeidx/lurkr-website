...
<sub>Make sure to replace `{guildId}` and `{userId}` with the actual guild and user IDs.</sub>
...
You can make 120 requests per minute per API key.
...
<ExternalLink className="text-primary" href={SUPPORT_SERVER_INVITE} target="_blank" rel="noopener noreferrer">Discord support server</ExternalLink>.
