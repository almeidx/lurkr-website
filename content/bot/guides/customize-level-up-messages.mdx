...
Use the `/config set` command to customize the level up message.
...
Not to worry though, as you can set where Lurkr will send the Level-Up message. To set this up, use the `/config set` command to configure the level up message destination.
...
Use the select menu to choose where to send level up messages.
...
Use the `/config set` command to set specific announcement levels.
...
Use the `/config set` command to set minimum leveling announcement threshold.
...
Use the `/config set` command to set factor for leveling announcements.
...
Use the `/config toggle` command to toggle only announcing level ups together with role rewards.
