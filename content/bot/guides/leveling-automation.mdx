...
description: Streamline your moderation and member engagement with automatic level adjustments, including resets for banned members
---
...
Use the `/config toggle` command for automatic level reset option.
...
You can enter as many prefixes as your current server plan will allow. You can check the limits on the [Premium Perks page](/bot/information/premium-perks)!

Use the `/config set` command to set ignored bot prefixes.
...
Use the `/config set` command to set the server-wide rank card color.
...
Use the `/config toggle` command to enable leveling in threads.
