export const BASE_URL = "https://lurkr.gg";
export const SUPPORT_SERVER_INVITE = "https://discord.gg/XUQAnkq2vy";
export const PATREON_URL = "https://patreon.com/lurkrbot";
export const TOPGG_URL = "https://top.gg/bot/506186003816513538/vote?utm_source=lurkr&utm_medium=dashboard";
export const GITHUB_REPOSITORY_OWNER = "almeidx";
export const GITHUB_REPOSITORY_NAME = "lurkr-website";
export const GITHUB_REPOSITORY_URL = `https://github.com/${GITHUB_REPOSITORY_OWNER}/${GITHUB_REPOSITORY_NAME}`;
export const UPDATES_CHANNEL_LINK = "https://discord.com/channels/705009450855039039/951585846367248424";
export const BOT_INVITE =
	"https://discord.com/oauth2/authorize?client_id=506186003816513538&scope=bot%20applications.commands&permissions=276220472384";
